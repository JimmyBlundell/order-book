...
const Home: React.FC = () => {
      ...
      if (userId) {
        try {
          const { data } = await Axios.get(
            `http://localhost:8000/getFeeds/${userId}`,
            {}
          );
  ...
  const addRssFeed = async () => {
    if (!userId) {
      alert(
        'Missing user information from local storage. Try logging in again'
      );
      return;
    }
    try {
      await Axios.post('http://localhost:8000/addRssFeed', {
        ...
        setRssFeeds([...rssFeeds, { name: rssFeedName, url: rssFeedUrl }])
      );
    } catch (error) {
      console.error(error);
    }
  ...
  const deleteRssFeed = async (url: string) => {
    ...
    try {
      await Axios.delete(
        `http://localhost:8000/deleteRssFeed/${userId}/${encodedUrl}`
      ).then(() => {
              ...
              <>
                <div className={'grid-cell'}>
                  ...
                  >
                    ...
                    <Dropdown.Item onClick={() => deleteRssFeed(feed.url)}>
                      Delete
                    </Dropdown.Item>
                  </DropdownButton>
                </div>
